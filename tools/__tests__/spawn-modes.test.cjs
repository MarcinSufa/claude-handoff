...
test('compact mode never calls openers and says compaction will reset the context', () => {
  const r = spawn({ mode: 'compact', prompt: 'hi', cwd: '/p', doc: '/p/.claude/handoff/auto/sX/HANDOFF.md', openers: {
    focus: () => { throw new Error('unreached') },
    uri: () => { throw new Error('unreached') },
    terminal: () => { throw new Error('unreached') },
  } })
  ...
  assert.equal(r.message, 'State saved to /p/.claude/handoff/auto/sX/HANDOFF.md. Compaction will reset the context; continue.')
  assert.doesNotMatch(r.message, /\/clear/)
  assert.doesNotMatch(r.message, /close (this|the old)/i)
})
