...
1. **Stop guard.** At the end of every turn `hooks/stop-context-guard.cjs` reads the newest assistant `usage` line of this session from the transcript. Above the save threshold (`150000` tokens) with no fresh snapshot for this session, it returns `{"decision":"block"}` with the token count and the instruction to save; the agent cannot end the turn until it has. Two blocks per session and compaction epoch at most, then it fails open so a session is never stranded.
...
- **`Stop` guard** (`hooks/stop-context-guard.cjs`): above the save threshold with no fresh snapshot, blocks the end of the turn with the save instruction; with a snapshot older than `HANDOFF_STALE_TOKENS` it asks for a refresh. Two blocks per session and epoch, then fail-open. Honours `stop_hook_active`, ignores every other hook event, and stays silent on a missing transcript or malformed input.
...
- **A manual `/handoff` marker left pending through a compaction is still there afterwards.** Compaction never consumes it (that's the auto-compact snapshot's job), so a later plain startup in the same project can still resume it, even though it predates the compaction. Type `/handoff` again, or clear the marker, if that stale resume is not what you want.
